define(['react'], function(React){
	...
	Common.createSelect = function(name, list, selectedDetail, filter, includeNone, defaults){
	    ...
	    for (var i = 0; i < list.length; i++) {
	        var option = list[i];
	        options.push(
	            React.createElement("option", {key: i, value: option._id}, option.typeName)
	        );
	    }
	    if(includeNone){
	    	 options.unshift(
            React.createElement("option", {key: "", value: ""}, "Ingen")
        );
	    }
		    ...
		    for (var i = 0; i < defaults.length; i++) {
					...
					options.unshift(
						React.createElement("option", {key: options.length, value: option.id}, option.name)
					);
				}
			}
	    ...
	    return (
	    		React.createElement("select", {name: name, value: selectedDetail, 
	...
	Common.createDetailSelect = function(name, list, selectedDetail, filter, includeNone, defaults){
	    ...
	    for (var i = 0; i < list.length; i++) {
	        var option = list[i];
	        options.push(
	            React.createElement("option", {key: i, value: option._id}, option.description)
	        );
	    }
	    if(includeNone){
				options.unshift(
